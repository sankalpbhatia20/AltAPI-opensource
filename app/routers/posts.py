...
def summarize_financial_statement(ticker: str = Path(..., description = "Enter the US company ticker name : "), financial_statement: str = Path(..., description = "Enter the financial statement you want a summary for (balance_sheet, cashflow or income_statement) : "), frequency: str = Path(..., description = "Enter the frequency of the statements (quarterly or annual): "), db: Session = Depends(get_db)):
    try:
        #print(current_user.email)
